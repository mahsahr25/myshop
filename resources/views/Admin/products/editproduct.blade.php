    ...
    <div class="container">
    	<div class="row">
    	    <div class="col-xs-12">
        	    <div class="form-wrap">
                <h1>Edit product</h1>
                    <form role="form" action="{{url('admin/product/'.$product->id)}}" method="post" id="login-form" autocomplete="off" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label for="email" class="sr-only">Id</label>
                            <p>Id</p><input type="text" name="id"  class="form-control" placeholder="{{$product->id}}" value="{{$product->id}}">
                        </div>
                        <div class="form-group">
                            <label for="key" class="sr-only">Password</label>
                            <p>Name</p><input type="text" name="name"  class="form-control" placeholder="{{$product->name}}" value="{{$product->name}}">
                        </div>
                        <div class="form-group">
                                <label for="key" class="sr-only">Password</label>
